...
type BenchmarkResults = {
  ...
  emotion: BenchmarkStats;
  iterations: number;
  warmups: number;
};

export default function EmotionComponentsScreen() {
  // Separate states for left (kstyled) and right (emotion)
    ...
    <View style={styles.container}>
      <ScrollView>
        <View style={styles.header}>
          <Text style={styles.title}>kstyled vs emotion</Text>
          <Text style={styles.subtitle}>
            Compare kstyled (compile-time) with emotion (runtime)
          </Text>
        </View>
        ...
        <View style={styles.splitView}>
          ...
          <View style={styles.column}>
            <View style={styles.columnHeaderRight}>
              <Text style={styles.columnHeaderText}>emotion</Text>
            </View>
            <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
              {benchmarkType === 'static'
                ? rightStaticCards.map((card) => (
                    <EmotionComponents.StaticCardRenderer key={card.id} data={card} />
                  ))
                : rightCards.map((card, index) => (
                    <EmotionComponents.DynamicCard key={card.id} data={card} index={index} />
                  ))}
            </ScrollView>
          </View>
        </View>
            ...
            {benchmarkResults && (
              <Text
                style={[
                  ...
                  {
                    color:
                      benchmarkResults.kstyled.median < benchmarkResults.emotion.median
                        ? '#007AFF'
                        : '#E91E63',
                  },
                ]}
              >
                ...
                {Math.abs(
                  ((benchmarkResults.kstyled.median - benchmarkResults.emotion.median) /
                    benchmarkResults.emotion.median) *
                    100,
                ).toFixed(1)}
                %{' '}
                {benchmarkResults.kstyled.median < benchmarkResults.emotion.median
            ...
            >
              {benchmarking ? (
                ...
                <>
                  ...
                  {/* emotion Statistics */}
                  <View style={styles.statsSection}>
                    <Text style={[styles.statsTitle, { color: '#E91E63' }]}>emotion</Text>
                    <View style={[styles.statsBox, { backgroundColor: '#FFF0F5' }]}>
                      <View style={styles.statRow}>
                        <Text style={styles.statLabel}>Median:</Text>
                        <Text style={styles.statValue}>
                          {benchmarkResults.emotion.median.toFixed(2)}ms
                        </Text>
                      </View>
                      <View style={styles.statRow}>
                        <Text style={styles.statLabel}>Mean (Average):</Text>
                        <Text style={styles.statValue}>
                          {benchmarkResults.emotion.mean.toFixed(2)}ms
                        </Text>
                      </View>
                      <View style={styles.statRow}>
                        <Text style={styles.statLabel}>Min / Max:</Text>
                        <Text style={styles.statValue}>
                          {benchmarkResults.emotion.min.toFixed(2)}ms /{' '}
                          {benchmarkResults.emotion.max.toFixed(2)}ms
                        </Text>
                      </View>
                      <View style={styles.statRow}>
                        <Text style={styles.statLabel}>P95 (95th percentile):</Text>
                        <Text style={styles.statValue}>
                          {benchmarkResults.emotion.p95.toFixed(2)}ms
                        </Text>
                      </View>
                      <View style={styles.statRow}>
                        <Text style={styles.statLabel}>Std Deviation:</Text>
                        <Text style={styles.statValue}>
                          ±{benchmarkResults.emotion.stdDev.toFixed(2)}ms
                        </Text>
                      </View>
                    </View>
                  </View>
